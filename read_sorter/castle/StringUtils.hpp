...
public:
	...
	~StringUtils();
	template<typename C>
	static void string_multi_split(std::string& str, char const* delimiters, C& ret_array);
	template<typename C>
	static void c_string_multi_split(const std::string& str, char const* delimiters, C& ret_array);
	template<typename C>
	static bool is_not_containing_ambiguous_bases(const C& str);
	template<typename C>
	static bool is_not_containing_ambiguous_bases_in_range(const C& str, int64_t start_offset, int64_t end_offset);
	...
	template<typename C>
	static std::string get_longest_common_sub_string(const C& str1, const C& str2);
	template<typename C>
	static void replace_all_non_ACGT_with_N(C& str);
	static int64_t count_differences(const std::string& s1, const std::string& s2, size_t n);
	// trim from start
...
template<typename C>
void StringUtils::string_multi_split(std::string& str, char const* delimiters, C& ret_array) {
	ret_array.clear();
	C output;
...
template<typename C>
void StringUtils::c_string_multi_split(const std::string& str, char const* delimiters, C& ret_array) {
	ret_array.clear();
	C output;
	std::bitset<255> delims;
	while (*delimiters) {
...
template<typename C>
bool StringUtils::is_not_containing_ambiguous_bases_in_range(const C& str, int64_t start_offset, int64_t end_offset) {
	const int64_t max_id = str.size();
	for (int64_t base_id = start_offset; base_id < end_offset && base_id < max_id; ++base_id) {
		if ('N' == str[base_id]) {
			return false;
		}
	}
...
template<typename C>
std::string StringUtils::get_longest_common_sub_string(const C& str1, const C& str2) {
//	std::cout << str1.size() << "/" << str2.size() << "\n";
	...
	for (std::set<char *>::const_iterator it = res.begin(); it != res.end(); it++) {
		res_str.append(1, **it);
	}
	for (uint64_t i = 0; i <= str1.length(); i++) {
		free(n[i]);
