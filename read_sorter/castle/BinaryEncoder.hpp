...
#include <read_sorter/castle/MurmurHash3.hpp>
...
const uint32_t MAX_PREMER_ID = 4;
typedef struct PremerBlockTupleType {
	uint32_t premer_block_id;
	uint64_t values[MAX_PREMER_ID];
//	uint32_t base_ids[MAX_PREMER_ID];
	// we set the values to the maximum value of uint64_t in order to find the premers (a premer is one of the minimizers)
	void set_to_max_values() {
//		values = 255;
//		value = UINT64_MAX;
		memset(values, 255, MAX_PREMER_ID * sizeof(uint64_t));
	}
	void invalidate_max_values() {
		for (uint32_t base_id = 0; base_id < MAX_PREMER_ID; ++base_id) {
			if (UINT64_MAX == values[base_id]) {
				values[base_id] = 0;
			}
		}
	}
...
// Abbreviation of the Pillar Minimer
template <class C>
struct Premer {
	uint64_t m_key;
	int64_t n_pos;
	C m_val;
	bool operator == (const Premer &rhs) const {
		return std::tie(this->m_key, this->n_pos, this->m_val) ==
			std::tie(rhs.m_key, rhs.n_pos, rhs.m_val);
	}
	bool operator < (const Premer &rhs) const {
		return std::tie(this->m_key, this->n_pos, this->m_val) <
				std::tie(rhs.m_key, rhs.n_pos, this->m_val);
	}
	bool operator > (const Premer &rhs) const {
		return std::tie(this->m_key, this->n_pos, this->m_val) >
				std::tie(rhs.m_key, rhs.n_pos, this->m_val);
	...
	friend std::ostream& operator<<(std::ostream& out, const Premer<C>& em) {
		std::stringstream ss;
		ss << em.m_key << "\t" << em.n_pos << "\t" << em.m_val;
		out << ss.str();
		return out;
	}
	friend std::istream& operator>>(std::istream& in, Premer<C>& em) {
		std::string m_key_str;
		std::string n_pos_str;
		std::string m_val_str;
		std::getline(in, m_key_str, '\t');
		std::getline(in, n_pos_str, '\t');
		std::getline(in, m_val_str, '\n');
		if(m_key_str.empty()|| n_pos_str.empty() || n_pos_str.empty()) {
			return in;
		}
		em.m_key = boost::lexical_cast<uint64_t>(m_key_str);
		em.n_pos = boost::lexical_cast<int64_t>(n_pos_str);
		em.m_val = m_val_str;
		return in;
	}
};
...
//	uint64_t largest_key;
	map<uint64_t, uint32_t> prev;
	map<uint64_t, uint32_t> next;
//	uint64_t prev_size() const {
//		return prev.size();
//	}
	uint64_t prev_size() const {
		return prev.size();
	...
	typedef google::dense_hash_map<uint64_t, PremerNode, KmerUInt64Hash> PremerMap;
	typedef google::dense_hash_set<uint64_t, KmerUInt64Hash> PremerSet;
class BinaryEncoder {
public:
	uint32_t readLength;
	uint32_t a;
	...
	template<typename C> void get_premers(const C& cs, const int64_t base_start_id, const int64_t base_end_id, PremerBlockTuple& premers,
	        const char target_c);
	template<typename C> Premer<C> get_premer(const C& cs, const int64_t base_start_id, const int64_t base_end_id, const char target_c);
	...
	template<typename C> void get_all_for_rev_premers(const C& cs, const int64_t base_start_id, const int64_t base_end_id, vector<uint64_t>& premers);
...
template<typename C> Premer<C> BinaryEncoder::get_premer(const C& cs, const int64_t base_start_id, const int64_t base_end_id, const char target_c) {
	uint64_t pre_temp;
	uint64_t a_premer_key = UINT64_MAX;
	int64_t temp_base_id = 0;
	const int64_t max_id = base_end_id - readLength;
	char c;
//	char c_reverse;
//	const char target_c_rerverse = getReverseBase(target_c);
	int64_t n_pos = base_start_id;
//	BinaryDecoder bd;
//	bd.set_k(readLength);
	for (int64_t base_id = base_start_id; base_id <= max_id; ++base_id) {
		temp_base_id = is_containing_low_complexity(cs, base_id, base_end_id);
...
//		}
		if (c != 'A') {
			pre_temp = encodeToTwoBitsLong(cs, base_id);
			if(a_premer_key > pre_temp) {
				a_premer_key = pre_temp;
				n_pos = base_id;
	//			std::cout << cs.substr(base_id, readLength) << "=>" << bd.decode(pre_temp) << "\n";
			}
		}
//		if (target_c_rerverse == c_reverse) {
//			pre_temp = encodeToTwoBitsLongReverse(cs, base_id);
//			a_premer_key = std::min(a_premer_key, pre_temp);
//			n_pos = base_id;
//		}
	}
...
//	}
	Premer<C> a_premer;
	a_premer.m_key = a_premer_key;
	a_premer.n_pos = n_pos;
	a_premer.m_val = cs;
...
template<typename C>
inline void BinaryEncoder::get_all_for_rev_premers(const C& cs, const int64_t base_start_id, const int64_t base_end_id, vector<uint64_t>& premers) {
	// A: premers[0]
	// C: premers[1]
	// G: premers[2]
	// T: premers[3]
...
//	char c;
	uint64_t pre_temp;
	int64_t temp_base_id = 0;
	premers[0] = std::numeric_limits<uint64_t>::max();
	premers[1] = std::numeric_limits<uint64_t>::max();
//	for(auto& a_premer: premers) {
//		a_premer = std::numeric_limits<uint64_t>::max();
//	}
	for (int64_t base_id = base_start_id; base_id <= max_id; ++base_id) {
		temp_base_id = is_containing_low_complexity(cs, base_id, base_id + readLength);
		if (-1 != temp_base_id) {
			base_id = temp_base_id;
			continue;
		}
//		c = cs[base_id];
		...
		pre_temp = encodeToTwoBitsLong(cs, base_id);
		if (premers[0] > pre_temp) {
			premers[0] = pre_temp;
		}
//		switch (c) {
//		case 'A':
//			if (premers[0] > pre_temp) {
////				cout << (boost::format("[BinaryEncoder.get_all_for_rev_premers] [FOR] c: %c, prev: %d, cur: %d\n") % c % premers[0].m_key % pre_temp).str();
//				premers[0] = pre_temp;
//			}
//			break;
//		case 'C':
//			if (premers[1] > pre_temp) {
////				cout << (boost::format("[BinaryEncoder.get_all_for_rev_premers] [FOR] c: %c, prev: %d, cur: %d\n") % c % premers[1].m_key % pre_temp).str();
//				premers[1] = pre_temp;
//			}
//			break;
//		case 'G':
//			if (premers[2] > pre_temp) {
////				cout << (boost::format("[BinaryEncoder.get_all_for_rev_premers] [FOR] c: %c, prev: %d, cur: %d\n") % c % premers[2].m_key % pre_temp).str();
//				premers[2] = pre_temp;
//			}
//			break;
//		case 'T':
//			if (premers[3] > pre_temp) {
////				cout << (boost::format("[BinaryEncoder.get_all_for_rev_premers] [FOR] c: %c, prev: %d, cur: %d\n") % c % premers[3].m_key % pre_temp).str();
//				premers[3] = pre_temp;
//			}
//			break;
//		default:
//			break;
//		}
		temp_base_id = base_id + readLength - 1;
//		c = getReverseBase(cs[temp_base_id]);
		pre_temp = encodeToTwoBitsLongReverse(cs, base_id);
		if (premers[1] > pre_temp) {
			premers[1] = pre_temp;
		}
...
//		case 'A':
//			if (premers[0] > pre_temp) {
////				cout << (boost::format("[BinaryEncoder.get_all_for_rev_premers] [REV] c: %c, prev: %d, cur: %d\n") % c % premers[0].m_key % pre_temp).str();
//				premers[0] = pre_temp;
//			}
//			break;
//		case 'C':
...
//	std::cout << "is_containing " << cs.substr(base_start_id, readLength) << "\n";
...
//	}
	for (int64_t base_id = max_id; base_id > base_start_id + 2; --base_id) {
		if ((cs[base_id - 1] == cs[base_id] && cs[base_id - 2] == cs[base_id - 1])) {
			return base_id;
		}
	}
	return -1;
